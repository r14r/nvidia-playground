...
class TestNVIDIA(unittest.TestCase):
    ...
    def test_set_default_model(self):
        self.assertEqual(self.n.set_default_model("llama"),"meta/llama-3.3-70b-instruct")
    ...
    def test_query_empty_choices_has_clear_error(self,OpenAI):
        ...
        with self.assertRaisesRegex(RuntimeError,"no completion choices"): self.n.query("hello",model="minimax")
    ...
    def test_content_text_string(self): self.assertEqual(NVIDIA._content_text("Hello world"),"Hello world")
class TestNVIDIAAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_query(self,AsyncOpenAI):
        resp=MagicMock();resp.choices=[MagicMock()];resp.choices[0].message.content="Async answer";AsyncOpenAI.return_value.chat.completions.create=AsyncMock(return_value=resp)
        self.assertEqual(await self.n.async_query("hello",model="minimax"),"Async answer")
    @patch("lib.nvidia.client.AsyncOpenAI")
    async def test_async_query_omits_zero_top_p(self,AsyncOpenAI):
        resp=MagicMock();resp.choices=[MagicMock()];resp.choices[0].message.content="ok";AsyncOpenAI.return_value.chat.completions.create=AsyncMock(return_value=resp)
        self.assertEqual(await self.n.async_query("hello",model="minimax",top_p=0),"ok")
        self.assertNotIn("top_p",AsyncOpenAI.return_value.chat.completions.create.call_args.kwargs)
    @patch("lib.nvidia.client.AsyncOpenAI")
    async def test_async_query_empty_choices_has_clear_error(self,AsyncOpenAI):
        resp=MagicMock();resp.choices=[];AsyncOpenAI.return_value.chat.completions.create=AsyncMock(return_value=resp)
        with self.assertRaisesRegex(RuntimeError,"no completion choices"): await self.n.async_query("hello",model="minimax")

...
class NVIDIA:
    ...
    async def async_query(
        ...
        **kwargs: Any,
        ...
        """Run a non-streaming query with the asynchronous OpenAI client."""
        ...
        **kwargs: Any,
        ...
        for number, chunk in enumerate(completion, start=1):
            ...
            if include_raw:
                ...
                event["raw"] = dump() if callable(dump) else None
            previous = now
            yield event
    ...
    async def async_stream_events(
        ...
        **kwargs: Any,
        ...
        """Stream events with native async I/O using AsyncOpenAI."""
        info, client = self._async_client_for(model)
        started = time.perf_counter()
        previous = started
        completion = await client.chat.completions.create(

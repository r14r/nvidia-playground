...
def get_nvidia() -> NVIDIA:
    if not MODELS_FILE.is_file():
        raise NVIDIAError(f"Model catalog not found: {MODELS_FILE}. Run `just models` first.")
    return _cached_nvidia(str(MODELS_FILE), MODELS_FILE.stat().st_mtime_ns)
...
def model_ids(nvidia: NVIDIA) -> list[str]:
    return [str(model.get("id") or model.get("model")) for model in nvidia.models(chat_only=True, safe=True)]


DEFAULT_SYSTEM_PROMPT = "You are a helpful technical assistant."
DEFAULT_USER_PROMPT = "**Describe advanced Streamlit features in three sentences per feature. No further information or additional details.**"
OLD_DEFAULT_USER_PROMPTS = {"Explain HTTP streaming in Python and show a small practical example."}
...
def require_nvidia() -> NVIDIA:
    try:
        ...
        st.error(str(exc))
        st.info("Generate the catalog with `nvidia-cli models --list --details --with-api-key --json --save models.json`.")
        st.stop()
        raise
